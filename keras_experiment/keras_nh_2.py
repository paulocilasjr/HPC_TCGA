...
def split_dataset_randomly(data, split_ratios=[0.7, 0.1, 0.2], seed=42):
...
if __name__ == "__main__":
    ...
    model.save('final_model.h5')  # Save the final model in HDF5 format
    print("Final trained model saved as 'final_model.h5'", flush=True)
    
    # Save training and validation metrics
    train_metrics = {k: v[-1] for k, v in history.history.items()}
    save_metrics(train_metrics, "keras_nh_2_1.txt")

    # Evaluate model on test dataset
    test_metrics = evaluate_model(model, test_dataset)

    # Save test metrics
    save_metrics(test_metrics, "keras_nh_2_1.txt")
    print("Metrics saved to keras_nh_2.txt", flush=True)
